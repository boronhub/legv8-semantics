...
class SourceTarget(Target):
    ...
    def build(self, output_dir: Path, deps: dict[str, Path], args: dict[str, Any], verbose: bool) -> None:
        shutil.copytree(self.SRC_DIR / 'legv8-semantics', output_dir / 'legv8-semantics')
...
class KompileTarget(Target):
    ...
    def build(self, output_dir: Path, deps: dict[str, Path], args: dict[str, Any], verbose: bool) -> None:
        kompile_args = self._kompile_args(deps['legv8-semantics.source'])
...
class KLLVMRuntimeTarget(Target):
    def build(self, output_dir: Path, deps: dict[str, Path], args: dict[str, Any], verbose: bool) -> None:
        compile_runtime(
            definition_dir=deps['legv8-semantics.llvm'],
            target_dir=output_dir,
            verbose=verbose,
        )

    def deps(self) -> tuple[str, ...]:
        return ('legv8-semantics.llvm',)
...
__TARGETS__: Final = {
    ...
    'llvm': KompileTarget(
        lambda src_dir: {
            'main_file': src_dir / 'legv8-semantics/legv8.md',
            'syntax_module': 'LEGV8',
